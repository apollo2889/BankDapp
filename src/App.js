...
function App() {
  ...
  const contractAddress = "0x6ccc6AE8baA4A61414712181479e63895431D76c";
  ...
  const customerBalanceHandler = async () => {
    try {
      if (window.ethereum) {
        ...
        let balance = await bankContract.getUserBalance();
  ...
  const deposityMoneyHandler = async (event) => {
    try {
      ...
      if (window.ethereum) {
        ...
        const txn = await bankContract.depositBalance({ value: ethers.utils.parseEther(inputValue.deposit) });
  ...
  const withDrawMoneyHandler = async (event) => {
    try {
      ...
      if (window.ethereum) {
        ...
        const txn = await bankContract.withdrawBalance(myAddress, ethers.utils.parseEther(inputValue.withdraw));
      ...
      </section>
      {
        isBankerOwner && (
          <section className="bank-owner-section">
            <h2 className="px-10 py-4 text-xl font-bold border-b-2 border-indigo-500">Bank Admin Panel</h2>
            <div className="p-10">
              <form className="form-style">
                <input
                  type="text"
